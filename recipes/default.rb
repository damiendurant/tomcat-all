...
ark 'tomcat' do
  ...
  notifies :create, "template[/etc/logrotate.d/tomcat#{node['tomcat-all']['user']}]", :immediately
end

# Log rotation (catalina.out)
template "/etc/logrotate.d/tomcat#{node['tomcat-all']['user']}" do
...
template "/etc/init.d/tomcat#{node['tomcat-all']['user']}" do
...
service "tomcat#{node['tomcat-all']['user']}" do
  action [:enable, :start]
end
